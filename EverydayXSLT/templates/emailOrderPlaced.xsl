		...
		<html>
			...
			<body>
				<div id="main">
					...
					<p>Thank you for placing an order with us. The following items have been ordered and are awaiting processing.</p>
		
					<div class="indent">
						<xsl:call-template name="renderOrderItems">          
							<xsl:with-param name="orderItems" select="OrderItems"/>		 
							<xsl:with-param name="searchStatus" select="'SHIP,ONORD'" />
						</xsl:call-template>			
					</div>
